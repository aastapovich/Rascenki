...
Provides a small `PriceViewHelper` class that wires tooltip-on-hover,
double-click to open full name, and selection handler for the
`ttk.Treeview` used to display `price.csv` contents.

RU: Помощники для взаимодействия с Treeview во вкладке просмотра прайса.
Класс `PriceViewHelper` подключает тултип при наведении, обработчик
двойного клика для показа полного наименования и обработчик выбора строки.
"""
